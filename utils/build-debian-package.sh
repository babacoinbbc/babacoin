...
PACKAGE_VERSION=$($SRC_PATH/src/babacoind --version | grep version | cut -d' ' -f4 | tr -d v)
...
# Copy binaries
cp $SRC_PATH/src/babacoind $DEB_BIN
cp $SRC_PATH/src/babacoin-cli $DEB_BIN
cp $SRC_PATH/util/fetch-params.sh $DEB_BIN/babacoin-fetch-params
# Copy docs
...
# Copy manpages
cp $SRC_DOC/man/babacoind.1 $DEB_MAN
cp $SRC_DOC/man/babacoin-cli.1 $DEB_MAN
cp $SRC_DOC/man/babacoin-fetch-params.1 $DEB_MAN
# Copy bash completion files
cp $SRC_PATH/contrib/babacoind.bash-completion $DEB_CMP/babacoind
cp $SRC_PATH/contrib/babacoin-cli.bash-completion $DEB_CMP/babacoin-cli
# Gzip files
gzip --best -n $DEB_DOC/changelog
gzip --best -n $DEB_DOC/changelog.Debian
gzip --best -n $DEB_MAN/babacoind.1
gzip --best -n $DEB_MAN/babacoin-cli.1
gzip --best -n $DEB_MAN/babacoin-fetch-params.1

cd $SRC_PATH/contrib

# Create the control file
dpkg-shlibdeps $DEB_BIN/babacoind $DEB_BIN/babacoin-cli
