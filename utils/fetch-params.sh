...
if [ "$uname_S" = "Darwin" ]; then
    PARAMS_DIR="$HOME/Library/Application Support/BabacoinParams"
else
    PARAMS_DIR="$HOME/.babacoin-params"
fi
...
DOWNLOAD_URL="https://github.com/Raptor3um/babacoin/releases/tag/1.2.15.3"
...
# fetch methods can be disabled with BBC_DISABLE_SOMETHING=1
BBC_DISABLE_WGET="${BBC_DISABLE_WGET:-}"
#BBC_DISABLE_IPFS="${BBC_DISABLE_IPFS:-}"
BBC_DISABLE_CURL="${BBC_DISABLE_CURL:-}"

LOCKFILE=/tmp/fetch_params.lock

fetch_wget() {
    if [ -z "$WGETCMD" ] || ! [ -z "$BBC_DISABLE_WGET" ]; then
        return 1
    fi
...
#fetch_ipfs() {
#    if [ -z "$IPFSCMD" ] || ! [ -z "$BBC_DISABLE_IPFS" ]; then
#        return 1
#    fi
...
fetch_curl() {
    if [ -z "$CURLCMD" ] || ! [ -z "$BBC_DISABLE_CURL" ]; then
        return 1
    fi
...
Failed to fetch the Babacoin parameters!
...
This script will fetch the Babacoin SNARK parameters and verify their
...
EOF
    ...
    then
        ...
        cat >> "$README_PATH" <<EOF
This directory stores common Babacoin SNARK parameters. Note that it is
