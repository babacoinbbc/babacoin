...
{
    const char* pszTimestamp = "BabaCoin Genesis";
    const CScript genesisOutputScript = CScript() << ParseHex("047af7943eeb12bf829056be6478e26345d188fa54316366b554c20df9ccec6e9a7456d3860bf68efa9e00a05e9997f881da7799387d2b9449fb71473ff4a67bad") << OP_CHECKSIG;
...
public:
    CMainParams() {
        ...
        consensus.nPowTargetTimespan = 24 * 60 * 60; // Babacoin: 1 day
        consensus.nPowTargetSpacing = 2 * 60; // Babacoin: 2 minutes
        ...
        // Value derived from block 925000 chainwork on mainnet (Apr 2026).
        // This prevents node from accepting an alternate chain with less work.
        consensus.nMinimumChainWork = uint256S("0x000000000000000000000000000000000000000000000000000027e9d661d3c6");

        // By default assume that the signatures in ancestors of this block are valid.
        // Set to checkpoint at block 925000 to skip signature validation for known-good history.
        consensus.defaultAssumeValid = uint256S("0x1eed5b11760ec0d11efac61b2d0a7429622c2ddcf2b365dadf6e6e19901144f7"); // 925000
        ...
        vSeeds.emplace_back("seed02.babacoin.network");
        vSeeds.emplace_back("seed03.babacoin.network");
        vSeeds.emplace_back("seed04.babacoin.network");
        vSeeds.emplace_back("seed05.babacoin.network");
        vSeeds.emplace_back("seed06.babacoin.network");
        vSeeds.emplace_back("seed07.babacoin.network");
        vSeeds.emplace_back("seed08.babacoin.network");
        vSeeds.emplace_back("seed09.babacoin.network");
        vSeeds.emplace_back("seed10.babacoin.network");


        // Babacoin addresses start with 'B'
        base58Prefixes[PUBKEY_ADDRESS] = std::vector<unsigned char>(1,25);
        // Babacoin script addresses start with '7'
        base58Prefixes[SCRIPT_ADDRESS] = std::vector<unsigned char>(1,102);
        // Babacoin private keys start with '7' or 'X'
        base58Prefixes[SECRET_KEY] =     std::vector<unsigned char>(1,205);
        // Babacoin BIP32 pubkeys start with 'xpub' (Bitcoin defaults)
        base58Prefixes[EXT_PUBLIC_KEY] = {0x04, 0x88, 0xAD, 0xE3};
        // Babacoin BIP32 prvkeys start with 'xprv' (Bitcoin defaults)
        base58Prefixes[EXT_SECRET_KEY] = {0x04, 0x88, 0xB2, 0x1E};
        // Babacoin BIP44 coin type is '5'
...
//        }
        ...
        checkpointData = {
          { {0,      uint256S("0x84de4877419c696744198422de2628087ae2270c73fb370ab4cfe2fe01061854")},
            {1000,   uint256S("0x4cc119a930ece39a22ea38e7fafe04cab24f9c0a7ebf4b655d4138c4498aa922")},
            {5000,   uint256S("0x8da656dc9dc0282823ad2e1c68dd3950b0da826029780d8b32fabcba5f23a20d")},
            {10000,  uint256S("0x3be12f4cdb52e11b8bfd0741c6d4c82a4ff8ecf627ba07319edcee6c7bff9497")},
            {25000,  uint256S("0x5cd28f17402a584a5c35f82a924d8548cf589d168f7ab86bb13ecca5c355f869")},
            {50000,  uint256S("0x9fcddc0337d4ca5365ed9ff876d815a8c12d3a660cf270dde7c966d25827ac2a")},
            {100000, uint256S("0x147e5fc63270e3ff9178307c3843bbca44307d28634d5c80c9cc40243773d438")},
            {200000, uint256S("0x00dd74ff5385d6ca8e52b0db25716db00beb9ad03ce2067a64fde33273345af9")},
            {300000, uint256S("0xfd352a0810f6dca251baddaec7a4cf35e59bda640bfc87610f07c0d87d140dc0")},
            {400000, uint256S("0x01f7a74539c55c88fb81cd177937b95f56e368c305923a6a0e253092519d29f0")},
            {500000, uint256S("0x5ee5908f5e5976d983653883d692b362790d3aeb50a1a6c528f135498b0d3d34")},
            {600000, uint256S("0xed3d64a04445c0c6bd3727792290e8683d56e0e16d20470f5c9509501386b29e")},
            {700000, uint256S("0xce2c81a37cbe5874dc29de19be936f9079415ea2fe1a1f429aeb5a206a77a879")},
            {800000, uint256S("0x3da5c7628c88e207069b86a27d9092cfc7c20a4cd02106880bf95827f0af6e54")},
            {900000, uint256S("0xafab0239ef52a7948c3a3b80ea5410cbaa73def766605fd9b0ea7ccbf6c2b0a4")},
            {925000, uint256S("0x1eed5b11760ec0d11efac61b2d0a7429622c2ddcf2b365dadf6e6e19901144f7")}
          }
	};

        chainTxData = ChainTxData{
          1777226752,   // * UNIX timestamp of last known number of transactions (Block 925000)
...
public:
    CTestNetParams() {
        ...
        consensus.powLimit = uint256S("00ffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffff"); // ~uint256(0) >> 20
        consensus.nPowTargetTimespan = 24 * 60 * 60; // Babacoin: 1 day
        consensus.nPowTargetSpacing = 60; // Babacoin: 1 minutes
        ...
        genesis = CreateGenesisBlock(1663169590, 52, 0x20001fff, 4, 5000 * COIN);
        consensus.hashGenesisBlock = genesis.GetHash();
        assert(consensus.hashGenesisBlock == uint256S("0x43426d103ddf224bcadab869363d21d44b496b08d4678f23c6adaf7af01fcbc8"));
        assert(genesis.hashMerkleRoot == uint256S("0x41db5581d19c0c1858ba5f54779285f37a6085225b90556502747c739c207a6e"));
        ...
        // Testnet Babacoin addresses start with '
        base58Prefixes[PUBKEY_ADDRESS] = std::vector<unsigned char>(1,26);
        // Testnet Babacoin script addresses start with '8' or '9'
        ...
        // Testnet Babacoin BIP32 pubkeys start with 'tpub' (Bitcoin defaults)
        base58Prefixes[EXT_PUBLIC_KEY] = {0x04, 0x35, 0x87, 0xCE};
        // Testnet Babacoin BIP32 prvkeys start with 'tprv' (Bitcoin defaults)
        base58Prefixes[EXT_SECRET_KEY] = {0x04, 0x35, 0x87, 0xCE};

        // Testnet Babacoin BIP44 coin type is '1' (All coin's testnet default)
        std::string strExtCoinType = gArgs.GetArg("-extcoinindex", "");
        ...
        std::vector<FounderRewardStructure> rewardStructures = {  {INT_MAX, 5}  };// 5% founder/dev fee forever
        consensus.nFounderPayment = FounderPayment(rewardStructures, 200, "B9E9r1gMZW2Y6aCngZHKsps6wHPCzMd7un");
        ...
        vSporkAddresses = {"BFhR8JLT7dXN7THtKmWWqWBTuU4gt5zrims"};
        nMinSporkKeys = 1;
        fBIP9CheckSmartnodesUpgraded = true;
...
public:
    CDevNetParams() {
       ...
       // consensus.DIP0003EnforcementHeight = 2; // DIP0003 activated immediately on devnet
        ...
        consensus.nPowTargetTimespan = 24 * 60 * 60; // Babacoin: 1 day
        consensus.nPowTargetSpacing = 2.5 * 60; // Babacoin: 2.5 minutes
        consensus.fPowAllowMinDifficultyBlocks = true;
        consensus.fPowNoRetargeting = false;
        consensus.nPowDGWHeight = 60;
        consensus.DGWBlocksAvg = 60;
        ...
        vFixedSeeds.clear();
        vSeeds.clear();
        //vSeeds.push_back(CDNSSeedData("babacoinevo.org",  "devnet-seed.babacoinevo.org"));

        // Testnet Babacoin addresses start with 'y'
...
public:
    CRegTestParams() {
       ...
       // consensus.DIP0003EnforcementHeight = 500;
        ...
        // Regtest Babacoin addresses start with 'y'
        base58Prefixes[PUBKEY_ADDRESS] = std::vector<unsigned char>(1,140);
        // Regtest Babacoin script addresses start with '8' or '9'
        base58Prefixes[SCRIPT_ADDRESS] = std::vector<unsigned char>(1,19);
        // Regtest private keys start with '9' or 'c' (Bitcoin defaults)
        base58Prefixes[SECRET_KEY] =     std::vector<unsigned char>(1,239);
        // Regtest Babacoin BIP32 pubkeys start with 'tpub' (Bitcoin defaults)
        base58Prefixes[EXT_PUBLIC_KEY] = {0x04, 0x35, 0x87, 0xCF};
        // Regtest Babacoin BIP32 prvkeys start with 'tprv' (Bitcoin defaults)
        base58Prefixes[EXT_SECRET_KEY] = {0x04, 0x35, 0x83, 0x94};

        // Regtest Babacoin BIP44 coin type is '1' (All coin's testnet default)
        nExtCoinType = 1;
