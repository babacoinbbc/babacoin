 ...
 *  We have 4 times smaller block times in Babacoin, so we need to push 4 times more invs per 1MB. */
...
void FindNextBlocksToDownload(NodeId nodeid, unsigned int count, std::vector<const CBlockIndex*>& vBlocks, NodeId& nodeStaller, const Consensus::Params& consensusParams) EXCLUSIVE_LOCKS_REQUIRED(cs_main)
{
    ...
    // LastCommonAncestor now returns a non-null fallback (shorter chain), but add
    // a safety check in case of unexpected null.
    if (state->pindexLastCommonBlock == nullptr) {
...
bool static AlreadyHave(const CInv& inv) EXCLUSIVE_LOCKS_REQUIRED(cs_main)
{
    ...
    /*
        Babacoin Related Inventory Messages
...
bool PeerLogicValidation::SendMessages(CNode* pto)
{
    const Consensus::Params& consensusParams = Params().GetConsensus();
    {
        ...
        // BABACOIN this code also handles non-TXs (Dash specific messages)
