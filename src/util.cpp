...
const char * const BITCOIN_CONF_FILENAME = "babacoin.conf";
const char * const BITCOIN_PID_FILENAME = "babacoind.pid";
...
public:
    ...
    static inline std::pair<bool,std::string> GetArg(const ArgsManager &am, const std::string& arg)
    {
        ...
        // argument value seen from the command line (so "babacoind -foo=bar
...
{
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\BabaCoin
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\BabaCoin
    // Mac: ~/Library/Application Support/BabaCoin
    // Unix: ~/.babacoin
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "BabaCoin";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/BabaCoin";
#else
    // Unix
    return pathRet / ".babacoin";
#endif
#endif
}
...
void ArgsManager::ReadConfigFile(const std::string& confPath)
{
    ...
    if (stream.good()) {
        ...
        // Create an empty babacoin.conf if it does not excist
...
std::string CopyrightHolders(const std::string& strPrefix, unsigned int nStartYear, unsigned int nEndYear)
{
    ...
    if (strprintf(COPYRIGHT_HOLDERS, COPYRIGHT_HOLDERS_SUBSTITUTION).find("Babacoin Core") == std::string::npos) {
        strCopyrightHolders += "\n" + strPrefix + strprintf(" %u-%u ", 2021, nEndYear) + "The Babacoin Core developers";
    }
